...
private:
  struct compare_name {
    bool operator()(const user_info_t &user_one, const user_info_t &user_two) const {

      if (user_one.user.name != user_two.user.name) {
        return user_one.user.name < user_two.user.name;
      } else if (user_one.total_call_duration_s != user_two.total_call_duration_s) {
        return user_one.total_call_duration_s > user_two.total_call_duration_s;
      }

      return user_one.user.number < user_two.user.number;
    }
  };
  ...
  struct compare_number {
    bool operator()(const user_info_t &user_one, const user_info_t &user_two) const {
      ...
      if (user_one.total_call_duration_s != user_two.total_call_duration_s) {
        return user_one.total_call_duration_s > user_two.total_call_duration_s;
      } else if (user_one.user.name != user_two.user.name) {
        return user_one.user.name < user_two.user.name;
      } else {
        return user_one.user.number < user_two.user.number;
      }
  ...
  std::set<user_info_t, compare_name> users_sorted_name{};
  std::map<std::string, user_info_t> user_number{};
  std::map<std::string, std::set<user_info_t, compare_number>> users_sorted_number{};
